...
class RequestHandler(ASTMProtocol):
    ...
    def prepareMindRay(self):
        self._whole_message += self._last_recv_data
        return ACK
        print(self._whole_message)


    def on_message(self):
        # Получаем IP-адрес клиента
        client_ip = self.client_info['host']

        # Передаем IP-адрес в RecordsDispatcher
        self.dispatcher.set_client_ip(client_ip)
        #self._previous_message = self._last_recv_data
        #allowed_ip = '192.168.1.52' #52
        #if self.client_info['host'] == allowed_ip:
            #print("hellloooooooo")
            #self.prepareMindRay()
            #if self._last_recv_data ==b'':
            #    self.handle_message(self._whole_message)
        ...
        if not self._is_transfer_state:
            ...
            try:
                #print("on message принял сообщение")
                self.handle_message(self._last_recv_data) #self._last_recv_data
                return ACK
            except Exception:
